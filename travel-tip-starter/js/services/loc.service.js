...
function query() {
    return storageService.query(DB_KEY)
        .then(locs => {
            console.log('Original locations:', locs)
            if (gFilterBy.txt) {
            ...
            console.log('Final locations:', locs)
            return locs
        })
}
...
function getLocCountByDateMap() {
    return storageService.query(DB_KEY)
        .then(locs => {
            ...
            const locCountByLastUpdatedMap = {
                today: 0,
                past: 0,
                never: 0,
                total: locs.length
            ...
            locs.forEach(loc => {
                ...
                const daysDifference = Math.floor((currentDate - updatedAt) / (1000 * 60 * 60 * 24))

                if (daysDifference === 0) {
                    locCountByLastUpdatedMap.today++
                } else if (daysDifference === 1) {
                    locCountByLastUpdatedMap.past++
                } else {
                    locCountByLastUpdatedMap.never++
                }
            })
